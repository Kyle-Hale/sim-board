                        ...
                        <div class="entry position-{{$entry.Position}}">
                            ...
                            <div class="time" style="display: flex; flex-direction: column; align-items: flex-end; gap: 2px;">
                                ...
                                <span style="font-size: 0.6em; color: rgba(255, 255, 255, 0.5); font-weight: 500; text-transform: uppercase; letter-spacing: 0.6px; line-height: 1.1; white-space: nowrap; margin-top: 2px;">
                                    <span style="color: rgba(255, 255, 255, 0.4);">ABS:</span> {{if $entry.ABS}}<span style="color: rgba(237, 28, 36, 0.8);">ON</span>{{else}}<span style="color: rgba(76, 175, 80, 0.9);">OFF</span>{{end}}<span style="margin: 0 0 0 1px; padding: 0 1px; color: rgba(255, 255, 255, 0.2);">|</span><span style="color: rgba(255, 255, 255, 0.4);">TRANS:</span> {{if $entry.AutoTransmission}}<span style="color: rgba(237, 28, 36, 0.8);">AUTO</span>{{else}}<span style="color: rgba(76, 175, 80, 0.9);">MANUAL</span>{{end}}<span style="margin: 0 0 0 1px; padding: 0 1px; color: rgba(255, 255, 255, 0.2);">|</span><span style="color: rgba(255, 255, 255, 0.4);">TC:</span> {{if $entry.TractionControl}}<span style="color: rgba(237, 28, 36, 0.8);">ON</span>{{else}}<span style="color: rgba(76, 175, 80, 0.9);">OFF</span>{{end}}
                                </span>
                                {{end}}
                            </div>
                        </div>
